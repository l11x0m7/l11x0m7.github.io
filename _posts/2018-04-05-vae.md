...
![](http://bloglxm.oss-cn-beijing.aliyuncs.com/vae-1.jpg)
...
![](http://bloglxm.oss-cn-beijing.aliyuncs.com/vae-epoch5_z.png)

epoch=5时，generator（decoder）的输出，可见效果还比较普通：

![](http://bloglxm.oss-cn-beijing.aliyuncs.com/vae-epoch5_x.png)

epoch=50时，z的分布（实验中z为二维），不同颜色代表不同数字类别，每个类别学习到了它自身的分布特征（虽然我们没有给类别标签，但是不同类别的样本还是有显著差异的），每种数字能形成一个簇：

![](http://bloglxm.oss-cn-beijing.aliyuncs.com/vae-epoch50_z.png)

epoch=50时，generator（decoder）的输出，效果比epoch=5时要好一些：

![](http://bloglxm.oss-cn-beijing.aliyuncs.com/vae-epoch50_x.png)

### tensorflow版本

训练1000个step，training loss如下：

![](http://bloglxm.oss-cn-beijing.aliyuncs.com/vae-trainloss.png)
