...
$$
L(\theta, \phi;x^{(i)})=KL(q_{\phi}(z|x^{(i)})||p_{\theta}(z))-{\mathbb{E}}_{z \sim q_{\phi}(z|x^{(i)})}(logp_{\theta}(x^{(i)}|z))
$$
...
假设$q_{\phi}(z \mid x^{(i)}) \sim {\Nu}(\mu, {\theta^2})$，$p_{\theta}(z)~{\Nu}(0, 1)$，则左侧的KL散度可以写成：
